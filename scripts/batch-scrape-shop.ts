 ...
 * Run: npx ts-node scripts/batch-scrape-shop.ts
 */
...
const OUTPUT_DIR = path.join(__dirname, '../docs/lore/sources/official-site/shop/scraped');
...
function saveProduct(product: ScrapedProduct) {
...
async function main() {
  ...
  for (let i = 0; i < SHOP_URLS.length; i += BATCH_SIZE) {
    ...
    console.log(`\n📦 Processing batch ${Math.floor(i/BATCH_SIZE) + 1}/${Math.ceil(SHOP_URLS.length/BATCH_SIZE)}`);
