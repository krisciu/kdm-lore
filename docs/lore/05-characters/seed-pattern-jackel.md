---
title: "Seed Pattern - Jackel"
category: characters
source: seed-pattern-jackel.txt
confidence: confirmed
updated: 2025-12-02
---

# Seed Pattern - Jackel

Once, Jackel was poised to strike the killing blow, when a brave survivor surged from behind, stealing her triumph. Frozen in thin-lipped silence, she could not offer even the most insincere accolades. A one-sided rivalry flourished in her embittered imagination. Never missing the chance to depart with her competitor in the hopes of glimpsing his bloody defeat. Finally, the seed of her bitterness bore fruit, the image of a rending weapon, one to leave her daring rival spineless.
For best experience, use with Gambler's Chest expansion.Artwork: Ley BowenSculpture: Team Death
Seed Pattern - Jackel is packaged in a graphic box that includes:
1 x Seed Pattern - Jackel hard plastic miniature
1 x Hard plastic Kingdom Death 30mm base and insert
1 x Spina Sickle seed pattern card
1 x Spina Sickle seed pattern gear card
1 x Living rules glossary card
1 x Large matte art print

---


---

## Related Images

- `official-site/images/shop/gamblers-chest-pattern3-1024x1024.png` (gamblers-chest)
- `rulebooks/extracted/images/gamblers-chest/Basic Pattern Gear - CARD BACK.png` (gear, gear)
- `rulebooks/extracted/images/gamblers-chest/Basic Pattern Recipes - CARD BACK.png` (gear, card)
- `rulebooks/extracted/images/game-assets/PATTERN GEAR.png` (gear, gear)
- `rulebooks/extracted/images/game-assets/Pattern-Gear.png` (gear, gear)

*Note: Images are stored in the sources directory. Use the image index for full metadata.*

## Source

This entry was extracted from official Kingdom Death product descriptions.

- **Source File:** `seed-pattern-jackel.txt`
- **Category:** concepts
- **Confidence:** Confirmed (official product description)

---

[← Back to Concepts](../_index.md)
